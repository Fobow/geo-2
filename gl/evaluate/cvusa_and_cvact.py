...
import gc
import copy
from gl.train.trainer import predict
...
def evaluate(config,
    ...
    if dist.get_rank() == 0: 
        print("\nExtract Features:")
    reference_features, reference_labels = predict(config, model, reference_dataloader, device, mode="sat") 
    query_features, query_labels = predict(config, model, query_dataloader, device, mode="grd")
