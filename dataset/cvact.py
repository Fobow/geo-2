...
from utils import load_and_preprocess_images, equirectangular_to_perspective

# align input transform
# config using config
class CVACTDatasetTrain(Dataset):
    ...
    def __init__(self,
        ...
        anuData = sio.loadmat(f'/opt/tiger/dino/GeoDiffuse/dataset/ACT_data.mat')
    ...
    def __getitem__(self, index):
        ...
        ground_views = []
        for yaw in [225, 315, 45, 135]:
            grd_view = equirectangular_to_perspective(query_img, fov=90, theta=yaw, phi=0, out_size=(416, 416), vertical_fov=180)
            ground_views.append(grd_view)
        ground_views = load_and_preprocess_images(ground_views)

        satellite_views = load_and_preprocess_images([reference_img])
        ...
        return query_img, reference_img, ground_views, satellite_views, label
...
class CVACTDatasetEval(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.img_type == "reference":
            views = load_and_preprocess_images([img])
        elif self.img_type == "query":
            ground_views = []
            for yaw in [225, 315, 45, 135]:
                grd_view = equirectangular_to_perspective(img, fov=90, theta=yaw, phi=0, out_size=(416, 416), vertical_fov=180)
                ground_views.append(grd_view)
            views = load_and_preprocess_images(ground_views)
